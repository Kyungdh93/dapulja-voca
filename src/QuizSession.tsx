...
function quizSessionReducer(state: State, action: Action) {
  ...
  action.payload.selected === newState.quizList[newState.currentIndex].answer
    ? (newState.correctCount += 1)
    : (newState.inCorrectCount += 1)
  newState.currentIndex += 1
  newState.isCompleted = newState.quizList.length === newState.currentIndex ? true : false
  return newState
}
...
function QuizSession() {
  ...
  const initState: () => Promise<State> = async () => {
    ...
    function QuizInitData(initialData: Array<Word>): Array<Quiz> {
      return initialData.map(({ text, meaning }, index) => {
        return {
          index,
          text,
          answer: meaning,
          selections: QuizSelections(meaning, initialData)
        }
      })
    }

    function QuizSelections(answer: string, wordList: Array<Word>): string[] {
      const tempList = wordList.filter(({ meaning }) => meaning !== answer)
      let wrongAnswerList: string[] = []
      while (wrongAnswerList.length < 2) {
        const randomNum: number = Math.floor(Math.random() * tempList.length)
        if (!wrongAnswerList.includes(tempList[randomNum].meaning))
          wrongAnswerList.push(tempList[randomNum].meaning)
      }
      return ShuffleList([...wrongAnswerList, answer])
    }

    function ShuffleList(selecttions: string[]): string[] {
      const randomList: string[] = []
      while (randomList.length < 3) {
        const randomNum: number = Math.floor(Math.random() * 3)
        if (!randomList.includes(selecttions[randomNum])) randomList.push(selecttions[randomNum])
      }
    ...
    return {
      ...
      inCorrectCount: 0,
      currentIndex: 0,
      quizList: QuizInitData(initialData),
      quizResults: []
    }
  }
